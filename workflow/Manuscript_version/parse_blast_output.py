...
here, we delete the matches which are less than 20 nucleotides long. For the matches which are 
between 20 and 24 nucleotides, we correct the number of mismatches to represent full query alignment. 
the new column "n_corrected_mismatches" represents the amount of mismatches for full query alignment. 

The script also removes duplicate hits of a probe against multiple transcript variants of the same gene. 
This would occur when using a transcriptome database, for instance. 
The hit with the highest percentage identity score to the probe is kept.
"""
...
df['probe_pair_id'] = df['probe_id'] #create extra probe_pair_id column to allow deletion of the RHS and LHS probe parts as a pair. 
df['probe_pair_id'] = df['probe_pair_id'].astype(str).str.split("_")
df['probe_pair_id'] = df['probe_pair_id'].str[0] + "_" + df['probe_pair_id'].str[1] + "_" + df['probe_pair_id'].str[2] + "_" + df['probe_pair_id'].str[5]
