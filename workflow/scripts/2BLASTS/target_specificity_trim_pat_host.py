...
Script adapted for 2 BLAST rounds.
"""
...
def trim_specificity(input_file, output_file):
    ...
    df = pd.read_csv(input_file, 
                    sep = "\t",
                    names = ["probe_id", "transcript_id", "qstart", "qend", 
                             "sstart", "send", "percentage_ident", "n_mismatches"]) 
...
def main(pat_input_file, host_input_file, pat_output_file, host_output_file):
    # Process pat input file
    trim_specificity(pat_input_file, pat_output_file)
    
    # Process host input file
    trim_specificity(host_input_file, host_output_file)

if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python3 target_specificity_trim_pat_host.py {pat_input_file} {host_input_file} {pat_output_file} {host_output_file}")
        sys.exit(1)

    pat_input_file = sys.argv[1]
    host_input_file = sys.argv[2]
    pat_output_file = sys.argv[3]
    host_output_file = sys.argv[4]

    main(pat_input_file, host_input_file, pat_output_file, host_output_file)