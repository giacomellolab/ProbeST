...
This script is used to generate a txt file to be used for Primer3 in the next rule in the Snakefile, for designing probes for 
...
Note: The headers can contain additional information for the user's own use. 


Command in Snakefile rule:
$ python3 primer3_input_design.py CDS_gene_targets.fa 

Output:
- primer3_input_snakemake.txt. A txt file containing primer3 instructions.
The sequence ID in the primer3 input file is a "_" separated ID of gene ID, gene name and transcript ID.
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise ValueError("Usage: python primer3_input_design.py CDS_gene_targets.fa output.txt")

    genes_of_interest = sys.argv[1]
    output_file = sys.argv[2]

    try:
        #Extract header and sequence information
        ...
        with open(output_file, 'w') as f:
            for index, row in seq_df.iterrows():
                ...
                f.write('PRIMER_INTERNAL_HAIRPIN_TH=15.0' + '\n')  # Threshold for hairpin stability (ΔG. Lower value is stricter. 47, 15
                f.write('PRIMER_INTERNAL_SELF_END_TH=15.0' + '\n')  # Threshold for 3' self-complementarity (ΔG) secondary structures
                f.write('PRIMER_INTERNAL_SELF_ANY_TH=15.0' + '\n')  # Threshold for any self-complementarity (ΔG) secondary structures
